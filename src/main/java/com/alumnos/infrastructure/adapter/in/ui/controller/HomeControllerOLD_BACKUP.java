...
@Controller
class HomeControllerOLD_BACKUP {

    private static final Logger LOG = LoggerFactory.getLogger(HomeControllerOLD_BACKUP.class);
    ...
    public HomeControllerOLD_BACKUP(AlumnoServicePort alumnoService, GrupoServicePort grupoService,
    ...
    private VBox crearVistaInformeConcentrado() {
        ...
        try {
            ...
            Label lblEspacio3 = new Label(" "); // Espacio para alinear con los otros labels
            ...
            btnExportar.setOnAction(event -> {
                if (tblInforme.getItems().isEmpty()) {
                    mostrarAlerta("Validación", "No hay datos para exportar. Primero realice una búsqueda.", Alert.AlertType.WARNING);
                    return;
                }
    ...
    /**
     * Exporta la tabla del informe de concentrado a un archivo Excel.
     *
     * @param tabla La tabla con los datos a exportar
     * @param grupo El grupo seleccionado
     * @param materia La materia seleccionada
     * @param parcial El parcial seleccionado
     */
    private void exportarInformeAExcel(TableView<java.util.Map<String, Object>> tabla, Grupo grupo, Materia materia, Integer parcial) {
        try {
            ...
            String nombreArchivo = String.format("informe_grupo_%s_materia_%s_parcial_%d.xlsx",
                grupo.getId(),
                materia.getNombre().replaceAll("[^a-zA-Z0-9]", "_"),
                parcial);
            fileChooser.setInitialFileName(nombreArchivo);
            ...
            try (Workbook workbook = new XSSFWorkbook()) {
                Sheet sheet = workbook.createSheet("Informe de Concentrado");
                ...
                // Obtener todas las columnas (incluyendo anidadas)
                List<TableColumn<java.util.Map<String, Object>, ?>> todasLasColumnas = new ArrayList<>();
                obtenerColumnasHoja(tabla.getColumns(), todasLasColumnas);

                LOG.info("Exportando {} columnas", todasLasColumnas.size());

                // Crear fila de encabezados
                Row headerRow = sheet.createRow(0);
                for (int i = 0; i < todasLasColumnas.size(); i++) {
                    org.apache.poi.ss.usermodel.Cell headerCell = headerRow.createCell(i);
                    headerCell.setCellValue(todasLasColumnas.get(i).getText());
                    headerCell.setCellStyle(headerStyle);
                }

                // Agregar datos
                ObservableList<java.util.Map<String, Object>> items = tabla.getItems();
                LOG.info("Exportando {} filas", items.size());

                for (int rowIndex = 0; rowIndex < items.size(); rowIndex++) {
                    Row row = sheet.createRow(rowIndex + 1);
                    java.util.Map<String, Object> rowData = items.get(rowIndex);

                    for (int colIndex = 0; colIndex < todasLasColumnas.size(); colIndex++) {
                        org.apache.poi.ss.usermodel.Cell dataCell = row.createCell(colIndex);
                        TableColumn<java.util.Map<String, Object>, ?> column = todasLasColumnas.get(colIndex);

                        // Obtener el valor de la celda
                        Object cellValue = null;
                        try {
                            var cellData = column.getCellObservableValue(rowData);
                            if (cellData != null) {
                                cellValue = cellData.getValue();
                            }
                        } catch (Exception e) {
                            LOG.debug("No se pudo obtener valor de celda para columna: {}", column.getText(), e);
                        }

                        // Determinar si la columna es de tipo check (contiene "Asistencia" o "Entregado")
                        boolean esColumnaCheck = column.getText().contains("Asistencia") ||
                                                column.getText().contains("Entregado") ||
                                                column.getText().contains("Entreg");
                        ...
                        if (cellValue == null || cellValue.toString().trim().isEmpty()) {
                            ...
                            if (esColumnaCheck) {
                                ...
                                dataCell.setCellStyle(emptyStyle);
                            } else {
                            ...
                            // Si es un checkbox (Boolean)
                            if (cellValue instanceof Boolean) {
                                dataCell.setCellValue((Boolean) cellValue ? "Sí" : "No");
                ...
                for (int i = 0; i < todasLasColumnas.size(); i++) {
                    try {
                        ...
                        sheet.setColumnWidth(i, Math.min(currentWidth + 500, 15000)); // Máximo 15000
                    } catch (Exception e) {
                        LOG.debug("No se pudo autoajustar columna {}", i);
                        sheet.setColumnWidth(i, 3000); // Ancho por defecto
                    }
                }
                ...
                mostrarAlerta("Éxito", "El archivo se exportó correctamente a:\n" + file.getAbsolutePath(),
                    Alert.AlertType.INFORMATION);

                LOG.info("Informe exportado exitosamente a: {}", file.getAbsolutePath());

            } catch (IOException e) {
                LOG.error("Error al crear archivo Excel", e);
                mostrarAlerta("Error", "No se pudo exportar el archivo: " + e.getMessage(),
                    Alert.AlertType.ERROR);
            }

        } catch (Exception e) {
            LOG.error("Error al exportar informe a Excel", e);
            mostrarAlerta("Error", "Error inesperado al exportar: " + e.getMessage(),
                Alert.AlertType.ERROR);
        }
    }

    /**
     * Obtiene todas las columnas de la tabla de forma recursiva (incluyendo columnas anidadas).
     */
    private void obtenerColumnasHoja(ObservableList<TableColumn<java.util.Map<String, Object>, ?>> columnas,
                                     List<TableColumn<java.util.Map<String, Object>, ?>> resultado) {
        for (TableColumn<java.util.Map<String, Object>, ?> columna : columnas) {
            if (columna.getColumns().isEmpty()) {
                // Es una columna hoja (sin hijos)
                resultado.add(columna);
            } else {
                // Tiene columnas hijas, procesar recursivamente
                obtenerColumnasHoja(columna.getColumns(), resultado);
            }
        }
    }
}
